...
document.getElementById('event-form').addEventListener('submit', function(event) {
    ...
    const eventLocation = document.getElementById('event-location').value;
    const eventDescription = document.getElementById('event-description').value;
    const eventFlyer = document.getElementById('event-flyer').files[0];
    ...
    reader.onload = function(e) {
        const eventData = {
            ...
            location: eventLocation,
            description: eventDescription,
            flyer: e.target.result // Store the base64 image data
        };
    ...
    reader.readAsDataURL(eventFlyer); // Convert the flyer image to a base64 string
});

// Function to display featured events on the page
function displayFeaturedEvents() {
    const events = JSON.parse(localStorage.getItem('events')) || { featured: [] };
    ...
    events.featured.forEach(event => {
        ...
        eventCard.innerHTML = `
            ...
            <p>Location: ${event.location}</p>
            <p>${event.description}</p>
            <img src="${event.flyer}" alt="${event.name} flyer" class="event-flyer" />
        `;
        featuredContainer.appendChild(eventCard);
    });
}
